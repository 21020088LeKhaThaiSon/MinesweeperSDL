...
bool loadMedia()
{
	...
	//Open the font
	gGamePlay = TTF_OpenFont("Font/DTM-Sans.ttf", 40);
	if (gGamePlay == NULL)
	{
	...
	else
	{
		...
		if (!gWinTexture.loadFromRenderedText("CONGRATULATION ^_^", textColor))
		{
	...
	else
	{
		//Render text
		SDL_Color textColor = { 140, 140, 140 };
		if (!gLoseTexture.loadFromRenderedText("GAME OVER :(", textColor))
		{
			cout << "Failed to render text texture!\n";
			success = false;
		}
...
void mineManager()
{
	...
	if (!gameOver && !isWinning)
	{
		...
		mineLeft.str("");
		if (countMineLeft > 1) {
			mineLeft << "Mines left: " << countMineLeft;
		}
		else {
...
void flagManager()
{
	...
	if (isWinning && !gameOver)
	{
		...
		gWinTexture.render((SCREEN_WIDTH - gWinTexture.getWidth()) / 2, 0);
	...
	if (gameOver)
	{
		...
		gLoseTexture.render((SCREEN_WIDTH - gLoseTexture.getWidth()) / 2, 0);
